...
public class test : MonoBehaviour
{
    ...
    public bool isFleeing = false;               // ‘лаг дл€ отслеживани€ состо€ни€ убегани€
    public float lastDamageTime;                 // ¬рем€ последнего удара
    public Transform player;                     // —сылка на игрока
    public NavMeshAgent agent;                   //  омпонент NavMeshAgent
    public Vector3 randomDestination;            // —лучайна€ точка назначени€
    public bool isChasing = false;               // ‘лаг дл€ отслеживани€ состо€ни€ преследовани€
    public bool shouldFollowPlayer = false;      // ‘лаг дл€ определени€, следовать ли за игроком после достижени€ случайной точки
    ...
    public void Start()
    {
        ...
        // ”станавливаем параметр скорости
        agent.speed = moveSpeed;

        // Ќаходим игрока по тегу "Player"
        player = GameObject.FindGameObjectWithTag("Player").transform;

        // «апускаем случайное блуждание
    ...
    public void Update()
    {
        ...
        {
            // ≈сли достигли новой случайной точки и не преследуем игрока, возвращаемс€ к преследованию
            if (!agent.isOnNavMesh && agent.remainingDistance < 0.5f && !isChasing)
            {
                isChasing = true;
                shouldFollowPlayer = true;

                // ≈сли не убегаем, устанавливаем точку к игроку
                if (!isFleeing)
                {
            ...
            // ≈сли следование за игроком разрешено, устанавливаем точку к игроку
            if (shouldFollowPlayer)
            {
                agent.SetDestination(player.position);
            }

            // ≈сли преследование активировано и игрок не в радиусе, возвращаемс€ к случайному блужданию
            if (isChasing && Vector3.Distance(transform.position, player.position) > chaseRadius)
            {
                ...
                // —брасываем флаг, чтобы можно было установить новое направление при следующем столкновении
